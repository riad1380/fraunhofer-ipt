var drawAxisLocation = d3.select("body").append("svg").attr("width",1000).attr("height",100);
var poly = d3.select("body").append("svg").attr("width",1000).attr("height",667).attr("id","polygon");
...
function colorRangeSelection(d,radioInputElements){
	...
	if(radioInputElements == 'Stress'){
		...
		var sixteenthInterval = fifteenthInterval + valueRange;
...
function drawPolygon(nodeFileName,elementFileName,radioInputElements){
	...
	d3.csv(nodeFileName,function(nodeData){
   	...
   	scaleXFactor = 800/differenceXValues;
   	scaleYFactor = 600/differenceYValues;
			...
			for(var i=0; i<d.length; i++){
				container.selectAll("g")
						...
						.on('mouseover',function (data){ 
							var tooltipText = '';
							if(radioInputElements == 'Stress')
								tooltipText = 'Element Number: ' + data [4] + "<br/>" + 'Stress: ' + (data [5]/1000000).toFixed(1) + ' MPa';
							else if(radioInputElements == 'Temperature')
								tooltipText = 'Element Number: ' + data [4] + "<br/>" + 'Temprature: ' + (data [6]).toFixed(1) + ' °C';
				...
				var circleAttributes = 
								circles.attr("cx", function (d){ return scaleX(d.xCoordinates)})
									...
									tooltip.transition()
										.duration(200)
										.style("display","inline");
									tooltip.html('Node Number: ' + d.NodeLabel + "<br/>"+ ' X: ' + parseFloat(d.xCoordinates*1000).toFixed(4) + ' mm' + "<br/>" +  ' Y: ' + parseFloat(d.yCoordinates*1000).toFixed(4) + ' mm')
...
function drawColorAxis(color,radioInputElements){
	...
	var axisScale = d3.scale.linear().domain([d3.min(colorAxis),d3.max(colorAxis)]).range([0,400]);
